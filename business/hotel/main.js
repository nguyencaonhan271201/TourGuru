...
const getHotelBookingDetails = () => {
    ...
    xhr.open(
        "GET",
        `../../api/bookings/info.php?getHotelsDetailsAdmin&booking_id=${planID}&user_id=gxRWAj31d4hQFJrHmwqwI4GlEOZ2&csrf=${csrf}&isAdmin=true`,
        true
    )
    xhr.onload = () => {
...
const getHotelBookingInfo = () => {
    const getBookingInfo = () => {
        ...
        xhr.open(
            "GET",
            `../../api/bookings/info.php?getHotelsInfoAdmin&booking_id=${planID}&user_id=gxRWAj31d4hQFJrHmwqwI4GlEOZ2&csrf=${csrf}&isAdmin=true`,
            true
        )
        xhr.onload = () => {
            if (xhr.status === 200 && xhr.readyState === 4) {
