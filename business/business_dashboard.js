...
const getRevenue = () => {
  ...
  xhr.onload = () => {
    if (xhr.status === 200 && xhr.readyState === 4) {
      ...
      result.forEach(cost => {
        let price = parseFloat(cost["total_cost"].replaceAll(",", "").substring(0, cost["total_cost"].length - 4));
        let currency = cost["total_cost"].substring(cost["total_cost"].length - 3, cost["total_cost"].length);
        let priceInUSD = price / ratesList[currency];
        sumRevenue += priceInUSD;
      })

      document.getElementById("revenue").innerHTML = `${sumRevenue.toFixed(2)} USD`;
    }
  };
  xhr.send();
};
