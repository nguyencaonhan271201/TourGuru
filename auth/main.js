...
let errorText = document.querySelector(
  "#sign-up-error-modal #modal-error-content"
);
let signInErrorText = document.querySelector(
  "#sign-in-error-modal #modal-error-content"
);
let user_id;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    window.location.search.includes("flight_confirmation") ||
...
const validateAndSignUp = () => {
  ...
  if (confirmPassword != password) {
    let passwordInvalid = signUpForm.querySelector("#password-invalid");
    let passwordInput = signUpForm.querySelector("#sign-up-password");
    let confirmPasswordInvalid = signUpForm.querySelector(
...
const updateToDatabaseProviderLogin = (data, token) => {
  ...
  xhr.open("POST", "../api/auth/auth.php", true);
  ...
  xhr.send(
    `googleSignUp&id=${data.id}&email=${data.email}&password=dummy&displayName=${data.displayName}&image=${data.image}&csrf=${csrf}`
  );
};
...
const updateToDatabaseNormalLogin = (data) => {
  ...
  xhr.send(
    `localSignUp&id=${data.id}&email=${data.email}&password=${data.password}&csrf=${csrf}`
  );
};
...
const getInfoFromServer = async () => {
  ...
  xhr.onload = () => {
    ...
    if (xhr.status === 200 && xhr.readyState === 4) {
      ...
      if (
        localStorage.getItem("fromFlight") &&
        localStorage.getItem("fromFlight") != "null"
      )
        location.replace("./../flights/confirmation/");
      else if (
        localStorage.getItem("hotelInfo") &&
        localStorage.getItem("hotelInfo") != "null"
      )
        location.replace(
          `./../hotels/info/?hotel=${
            JSON.parse(localStorage.getItem("hotelInfo"))["hotel"]["ID"]
          }`
        );
